# This is Section 7.1 Figure 4 in the Manuscript #
...
run_sim <- function(i, p) {
  ...
  # Scenario 1: One treatment but treatment effect increasing over time
  # This is for the online supplement data
  data1 <- data %>% 
    mutate(
      group = case_when(
  ...
  # Scenario 2: No pre-trends with treatment. This is for the manuscript Section XXX
  data2 <- data %>%
    mutate( 
  ...
  # Data 3 pre-trends no treatment. Did not use as no treatment effect. 
  data3 <- data %>%
    mutate(
      group = case_when(
        incorp %in% random_states[1:17] ~ 1997,
  ...
  # run it for simulations and store results in a dataset
  estimates <- map_dfr(1:3, get_est)
...
# run function 
sim2_estimates <- make_dist_plot(2, 
                  "Multi-Group Multi-Period Staggered Adoption Static Effect") # data3 from estimates 
sim2_estimates

# save
ggsave(sim2_estimates, filename = here::here("Figs_Tables", "StaggeredT_Static.png"), 
       dpi = 500,
       width = 6, height = 4)

#### 3. Single Adoption DID: Online Supplement ####
...
# Distribution Plot
sim1_estimates <- make_dist_plot(1, 
                  "Multi-Group Multi-Period Single Adoption Static Effect") # data1 from estimates 
